...
/// Hàm sử lý yêu cầu tại đường dẫn '/api/v1/check'
Response _checkHandler(Request req) {
  return Response.ok(json.encode({'message':'Chào Mừng bạn đến với ứng dụng web động'}),
  headers: _headers,
  );
}
...
Future<Response> _submitHandler(Request req) async {
  try {
    ...
    final response = {
      'message': 'Yêu cầu không hợp lệ. lỗi ${e.toString()}'
    };
